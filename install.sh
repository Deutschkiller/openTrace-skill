...
SKILL_BASE="$HOME/.config/opencode/skills"

mkdir -p "$SKILL_BASE/vtags-tracer"
sed "s|{{VTAGS_PATH}}|$VTAGS_PATH|g" "$VTAGS_DIR/skills/vtags-tracer/SKILL.md" > "$SKILL_BASE/vtags-tracer/SKILL.md"

mkdir -p "$SKILL_BASE/vcd-analyzer"
sed "s|{{VTAGS_PATH}}|$VTAGS_PATH|g" "$VTAGS_DIR/skills/vcd-analyzer/SKILL.md" > "$SKILL_BASE/vcd-analyzer/SKILL.md"
...
if [ -f "$SKILL_BASE/vtags-tracer/SKILL.md" ]; then
    echo -e "${GREEN}✓ vtags-tracer skill 已安装${NC}"
fi
if [ -f "$SKILL_BASE/vcd-analyzer/SKILL.md" ]; then
    echo -e "${GREEN}✓ vcd-analyzer skill 已安装${NC}"
fi

echo -e "\n${GREEN}=== 安装完成 ===${NC}\n"
echo -e "vtags 路径: $VTAGS_PATH"
echo -e "Skills 路径:"
echo -e "  - $SKILL_BASE/vtags-tracer/SKILL.md"
echo -e "  - $SKILL_BASE/vcd-analyzer/SKILL.md\n"
echo -e "快速开始:"
