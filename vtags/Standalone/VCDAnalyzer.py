...
class VCDAnalyzer:
    ...
    def parse(self):
        ...
        Returns:
            self (支持链式调用)
        """
        cleaned_path = self._preprocess_vcd()
        original_path = self.vcd_path

        try:
            if cleaned_path:
                self.vcd_path = cleaned_path
            self._vcd = VCDVCD(self.vcd_path)
        except ValueError as e:
            if "not enough values to unpack" in str(e):
                raise ValueError(
                    f"VCD file format error: {original_path}\n"
                    f"Details: {e}\n"
                    f"This may be caused by malformed VCD file generated by simulation tool.\n"
                    f"Suggestions:\n"
                    f"  1. Check simulation tool (e.g., Icarus Verilog) configuration\n"
                    f"  2. Try regenerating the VCD file\n"
                    f"  3. Use GTKWave to verify/fix the VCD file"
                )
            raise
        finally:
            self.vcd_path = original_path
    ...
    def _preprocess_vcd(self):
        """
        预处理 VCD 文件，跳过格式错误的行

        Returns:
            str: 临时清理后的文件路径，或 None（如果文件正常）
        """
        problematic_lines = []
        ...
        try:
            with open(self.vcd_path, "r", errors="ignore") as f:
                for i, line in enumerate(f, 1):
                    ...
                    if stripped and stripped[0] in "bBrR":
        ...
        warnings.warn(
            f"VCD file contains {len(problematic_lines)} malformed lines. "
            f"Creating cleaned temporary file (skipping bad lines).\n"
            f"First problematic line: {problematic_lines[0][0]}: {problematic_lines[0][1]}",
            UserWarning,
        )

        skip_set = set(line_num for line_num, _ in problematic_lines)

        fd, cleaned_path = tempfile.mkstemp(suffix=".vcd", prefix="vtags_cleaned_")

        try:
            with open(self.vcd_path, "r", errors="ignore") as fin:
